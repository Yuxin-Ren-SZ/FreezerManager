...
namespace fmgr::storage {
  namespace {
    ...
    [[nodiscard]] std::vector<SqliteMigration> default_migrations() {
      // SQL bodies live in src/storage/sqlite/migrations/*.sql, embedded at build
      // time into SqliteMigrationsEmbedded.h (k_sqlite_embedded_migrations). Each
      // migration is reviewable as its own file, and the checksum (blake2b over
      // "version:name:up_sql") stays byte-identical to the file content.
      std::vector<SqliteMigration> migrations;
      migrations.reserve(k_sqlite_embedded_migrations.size());
      for (const auto& embedded : k_sqlite_embedded_migrations) {
        migrations.push_back(SqliteMigration{.version = embedded.version,
...
CREATE TABLE IF NOT EXISTS schema_migrations (
  ...
  applied_at_unix_seconds INTEGER NOT NULL
      ...
      // (e.g. conformance tests) also get the correct schema. This DDL intentionally
      // mirrors migrations/0011_audit_events.sql; it is a CREATE ... IF NOT EXISTS
      // bootstrap (not part of any checksummed migration body), so it is kept inline
      // rather than embedded — the migration-files test guards the 0011 checksum.
      exec(handle, R"sql(
...
CREATE TRIGGER IF NOT EXISTS audit_events_no_delete
  ...
  void SqliteTransaction::note_phi_read(const std::string& entity_kind,
