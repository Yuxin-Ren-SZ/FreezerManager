...
namespace {
  ...
  protected:
    void SetUp() override {
      ...
      client_ = std::make_unique<AuthServiceClient>(fmgr::v1::AuthService::NewStub(channel));
    }
  ...
  TEST_F(AuthServiceClientTest, SubmitMfaSendsCodeWithBearer) {
    auto result = client_->submitMfa(QStringLiteral("tok-123"), QStringLiteral("654321"));
    ASSERT_TRUE(result.ok);
  ...
  TEST_F(AuthServiceClientTest, SubmitMfaFailureCarriesError) {
    ...
    auto result = client_->submitMfa(QStringLiteral("tok-123"), QStringLiteral("000000"));
    EXPECT_FALSE(result.ok);
