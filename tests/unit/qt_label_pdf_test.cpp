...
namespace {
  ...
  // Ids beginning with "bad-" fail (error injection); id "unplaced" resolves but
  // carries no barcode/box/position so the location-format fallback is exercised.
  ...
  public:
    grpc::Status GetSample(grpc::ServerContext* /*ctx*/, const fmgr::v1::GetSampleRequest* req,
      ...
      if (req->sample_id().rfind("bad-", 0) == 0) {
      ...
      if (req->sample_id() == "unplaced") {
        return grpc::Status::OK; // no barcode / box / position set
      }
  ...
  TEST_F(LabelPdfTest, RendersSingleLabel) {
    const auto m = pdf_->buildModel({QStringLiteral("s-1")}, QStringLiteral("tok"));
    ...
    const QByteArray bytes = pdf_->generate({QStringLiteral("s-1")}, QStringLiteral("tok"));
    EXPECT_TRUE(bytes.startsWith("%PDF"));
  }

  TEST_F(LabelPdfTest, RendersMultipleLabels) {
    const std::vector<QString> ids = {QStringLiteral("s-1"), QStringLiteral("s-2"),
                                      QStringLiteral("s-3")};
  ...
  TEST_F(LabelPdfTest, UnreadableSampleIsSkipped) {
    const auto m = pdf_->buildModel({QStringLiteral("bad-1")}, QStringLiteral("tok"));
    EXPECT_TRUE(m.labels.empty());

    const QByteArray bytes = pdf_->generate({QStringLiteral("bad-1")}, QStringLiteral("tok"));
    EXPECT_TRUE(bytes.startsWith("%PDF")); // still a valid (empty) sheet
  }

  TEST_F(LabelPdfTest, MixedSuccessAndFailureKeepsOnlyResolved) {
    const std::vector<QString> ids = {QStringLiteral("s-1"), QStringLiteral("bad-2"),
                                      QStringLiteral("s-3")};
    const auto m = pdf_->buildModel(ids, QStringLiteral("tok"));
    ASSERT_EQ(m.labels.size(), 2u);
    EXPECT_EQ(m.labels[0].name, QStringLiteral("Sample s-1"));
    EXPECT_EQ(m.labels[1].name, QStringLiteral("Sample s-3"));
  }

  TEST_F(LabelPdfTest, MissingOptionalFieldsStillRender) {
    const auto m = pdf_->buildModel({QStringLiteral("unplaced")}, QStringLiteral("tok"));
    ASSERT_EQ(m.labels.size(), 1u);
    EXPECT_EQ(m.labels[0].name, QStringLiteral("Sample unplaced"));
    EXPECT_TRUE(m.labels[0].barcode.isEmpty());
    // box_id and position_label both empty → "·" separator with blank sides.
    EXPECT_EQ(m.labels[0].location, QStringLiteral(" · "));

    const QByteArray bytes = pdf_->generate({QStringLiteral("unplaced")}, QStringLiteral("tok"));
    EXPECT_TRUE(bytes.startsWith("%PDF"));
  }

  // Nine labels overflow the 8-per-page Avery grid, forcing a second page.
  TEST_F(LabelPdfTest, OverflowSpillsToSecondPage) {
    std::vector<QString> ids;
    for (int i = 0; i < 9; ++i) {
      ids.push_back(QStringLiteral("s-%1").arg(i));
    }
    const auto m = pdf_->buildModel(ids, QStringLiteral("tok"));
    EXPECT_EQ(m.labels.size(), 9u);

    // Exercises the newPage() branch (i % kPerPage == 0 for i == 8) without crash.
    const QByteArray bytes = pdf_->generate(ids, QStringLiteral("tok"));
    EXPECT_TRUE(bytes.startsWith("%PDF"));
    EXPECT_FALSE(bytes.isEmpty());
  }
