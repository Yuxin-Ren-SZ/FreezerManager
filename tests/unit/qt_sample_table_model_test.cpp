...
TEST_F(SampleTableModelTest, SetScopeLoadsFirstPage) {
  ...
  EXPECT_EQ(
      model_->data(model_->index(0, SampleTableModel::kName), ::Qt::DisplayRole)
          .toString(),
      QStringLiteral("Sample 1"));
  EXPECT_EQ(model_->data(model_->index(0, SampleTableModel::kStatus),
                         ::Qt::DisplayRole)
                .toString(),
            QStringLiteral("Active"));
  EXPECT_EQ(model_->data(model_->index(0, SampleTableModel::kVolume),
                         ::Qt::DisplayRole)
                .toString(),
...
TEST_F(SampleTableModelTest, PagesInMoreRowsViaFetchMore) {
  ...
  EXPECT_EQ(model_->data(model_->index(1, SampleTableModel::kName),
                         ::Qt::DisplayRole)
                .toString(),
...
TEST_F(SampleTableModelTest, HeaderLabels) {
  EXPECT_EQ(model_
                ->headerData(SampleTableModel::kName, ::Qt::Horizontal,
                             ::Qt::DisplayRole)
                .toString(),
            QStringLiteral("Name"));
  EXPECT_EQ(model_
                ->headerData(SampleTableModel::kVolume, ::Qt::Horizontal,
                             ::Qt::DisplayRole)
                .toString(),
