...
namespace {
  ...
  public:
    ...
    grpc::Status GetSample(grpc::ServerContext* /*ctx*/, const fmgr::v1::GetSampleRequest* req,
      ...
      if (fail_get) {
        return {grpc::StatusCode::NOT_FOUND, "no such sample"};
      }
    ...
    bool seen_has_volume_used = false;
    double seen_volume_used = 0.0;
    std::string seen_checkout_volume_unit;
    std::string seen_checkout_reason;
    ...
    grpc::Status CheckoutSample(grpc::ServerContext* /*ctx*/,
      ...
      seen_has_volume_used = req->has_volume_used();
      seen_volume_used = req->volume_used();
      seen_checkout_volume_unit = req->volume_unit();
      seen_checkout_reason = req->reason();
      if (fail_checkout) {
    ...
    grpc::Status ExportSamplesCsv(grpc::ServerContext* /*ctx*/,
      ...
      if (fail_export) {
        return {grpc::StatusCode::PERMISSION_DENIED, "not your lab"};
      }
  ...
  TEST_F(SampleServiceClientTest, ListSamplesForwardsCursor) {
    SampleFilter filter;
    filter.lab_id = QStringLiteral("lab-1");
    auto result = client_->listSamples(QStringLiteral("tok-1"), filter, QStringLiteral("p2"));
    ASSERT_TRUE(result.ok);
  ...
  TEST_F(SampleServiceClientTest, CheckoutSamplePassesAction) {
    auto result = client_->checkoutSample(QStringLiteral("tok-1"), QStringLiteral("s-1"),
  ...
  TEST_F(SampleServiceClientTest, ExportSamplesCsvMapsContent) {
    auto result = client_->exportSamplesCsv(QStringLiteral("tok-1"), QStringLiteral("lab-1"),
  ...
  TEST_F(SampleServiceClientTest, ImportSamplesDryRunMapsRowReport) {
    auto result = client_->importSamples(QStringLiteral("tok-1"), QStringLiteral("lab-1"),
  ...
  TEST_F(SampleServiceClientTest, GetSamplePassesId) {
    auto result = client_->getSample(QStringLiteral("tok-1"), QStringLiteral("s-9"));
    ASSERT_TRUE(result.ok);
  ...
  TEST_F(SampleServiceClientTest, MoveSamplePassesDestination) {
    auto result = client_->moveSample(QStringLiteral("tok-1"), QStringLiteral("s-1"),
                                      QStringLiteral("box-2"), QStringLiteral("B3"));
    ASSERT_TRUE(result.ok);
  ...
  TEST_F(SampleServiceClientTest, ListSamplesForwardsItemTypeFilter) {
    SampleFilter filter;
    filter.lab_id = QStringLiteral("lab-1");
    filter.item_type_id = QStringLiteral("it-42");
    client_->listSamples(QStringLiteral("tok-1"), filter);
    EXPECT_TRUE(service_.seen_has_item_type);
    EXPECT_EQ(service_.seen_item_type_id, "it-42");
  }

  TEST_F(SampleServiceClientTest, GetSampleNotFoundCarriesError) {
    service_.fail_get = true;
    auto result = client_->getSample(QStringLiteral("tok-1"), QStringLiteral("s-9"));
    EXPECT_FALSE(result.ok);
    EXPECT_EQ(result.error, "no such sample");
    EXPECT_EQ(service_.seen_get_id, "s-9");
    EXPECT_TRUE(result.sample.id.isEmpty());
  }

  TEST_F(SampleServiceClientTest, ExportSamplesCsvPermissionDeniedCarriesError) {
    service_.fail_export = true;
    auto result = client_->exportSamplesCsv(QStringLiteral("tok-1"), QStringLiteral("lab-1"),
                                            /*include_archived=*/false);
    EXPECT_FALSE(result.ok);
    EXPECT_EQ(result.error, "not your lab");
    EXPECT_TRUE(result.csv.isEmpty());
  }

  TEST_F(SampleServiceClientTest, CheckoutSampleForwardsVolumeAndReason) {
    auto result = client_->checkoutSample(QStringLiteral("tok-1"), QStringLiteral("s-1"),
                                          fmgr::v1::CHECKOUT_ACTION_CHECKOUT, /*volume_used=*/0.25,
                                          QStringLiteral("mL"), QStringLiteral("aliquot for PCR"));
    ASSERT_TRUE(result.ok);
    EXPECT_EQ(service_.seen_checkout_id, "s-1");
    EXPECT_TRUE(service_.seen_has_volume_used);
    EXPECT_DOUBLE_EQ(service_.seen_volume_used, 0.25);
    EXPECT_EQ(service_.seen_checkout_volume_unit, "mL");
    EXPECT_EQ(service_.seen_checkout_reason, "aliquot for PCR");
  }

  TEST_F(SampleServiceClientTest, CheckoutSampleOmitsUnsetOptionalArgs) {
    auto result = client_->checkoutSample(QStringLiteral("tok-1"), QStringLiteral("s-1"),
                                          fmgr::v1::CHECKOUT_ACTION_CHECKOUT);
    ASSERT_TRUE(result.ok);
    EXPECT_FALSE(service_.seen_has_volume_used);
    EXPECT_TRUE(service_.seen_checkout_volume_unit.empty());
    EXPECT_TRUE(service_.seen_checkout_reason.empty());
  }
